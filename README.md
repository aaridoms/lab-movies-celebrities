...
# LAB | Mongoose Movies and Celebrity
...
As a summary of our CRUD journey, we will work on creating an app for movies and Celebrity.
...
**1. Add new Celebrity**\
**2. See the list of Celebrity**

In the second part of the application, when you already have a couple of Celebrity in your database, let's figure out what we will do regarding _the movie_ model. As we said, we will have a full CRUD on this model, which means the user can:

**3. Add new movies**\
**4. See the list of all movies**\
**5. See the details of a specific movie, including its cast of Celebrity**\
...
- **Routes**: In our `routes` folder, let's create separate files for our **Celebrity** and **movies**. The naming is up to you, but we will use the following: `routes/Celebrity.routes.js` and `routes/movies.routes.js`. You can add below starter router code to both of them and remember to link these two new files to either `app.js` or `routes/index.js` so your server has access to them.

```js
// starter code in both routes/Celebrity.routes.js and routes/movies.routes.js
...
- **Views**: To keep things nice and clean, we will also create separate folders for `Celebrity` and `movies`: `views/Celebrity` and `views/movies`. Also, we will create a couple of files in each folder:

  - `views/Celebrity/Celebrity.hbs`
  - `views/Celebrity/new-celebrity.hbs`
...
Our first step is to create the `Celebrity` model and add some Celebrity to our database.
...
- `catchPhrase` - String (every celebrity needs a good catchphrase. Well maybe not all of them have one in real life, but all of _our_ Celebrity will have a catchphrase. This can be pretty much anything.)
...
## Iteration #3: Adding New Celebrity

Now that we have defined _Celebrity_ model, let's make it so the user can **add new Celebrity to the database**.

<br>

| Route                 | HTTP Verb | Description                                                                                   |
| --------------------- | --------- | --------------------------------------------------------------------------------------------- |
| `/Celebrity/create` | GET       | Show a form to create a celebrity                                                             |
| `/Celebrity/create` | POST      | Send the data from the form to this route to create the celebrity and save it to the database |

<br>

### Steps we will follow in this iteration:

1. In the routes file (`routes/Celebrity.routes.js`) create the following GET route: `/Celebrity/create`
2. In that route we have to render the `Celebrity/new-celebrity` view
3. In that view file:
   - Add an `<h2>` for the page's heading.
   - Add a `<form>` tag that makes a POST request to `/Celebrity/create`.
...
4. Create the `/Celebrity/create` POST route in `routes/Celebrity.routes.js`.
5. In that route we have to **create** an instance of the `Celebrity` model (don't forget, we should get all the info from the form through _req.body_)
   - If there is an error, render the `Celebrity/new-celebrity` view so the user can try again and
   - If there is no error, redirect to the page with the list of Celebrity. This route will be created in the next iteration of `/Celebrity`
...
## Iteration #4: Listing Our Celebrity

Now, when we've got some Celebrity in the database, we can start working with them in our Express app. Let's **display a list of all the Celebrity**.
...
| Route          | HTTP Verb | Description          |
| -------------- | --------- | -------------------- |
| `/Celebrity` | GET       | Show all Celebrity |

<br>

### Steps we will follow in this iteration:

1. Create the `/Celebrity` GET route in `routes/Celebrity.routes.js`.
2. In the route:
   - Use `find()` method on the `Celebrity` model to retrieve all the Celebrity
   - If everything is okay, render the `Celebrity/Celebrity.hbs` view and pass the array of Celebrity into the view as a variable
   - If there's an error, catch it
3. In the `views/Celebrity/Celebrity.hbs` view file:
...
4. In the `views/index.hbs` (homepage) file:
   - Add a link that goes to the `/Celebrity` route.

<br>

**Celebrity - Done! At least for now.** 😉
...
- `genre` - String
- `plot` - String
- `cast` - Array of object IDs referencing the _Celebrity_ model (basically, the array of Celebrity' IDs)
...
- Create 2 new routes, one to render a page with the form on it, and one to send the data to after the form is filled out
  - In the GET route that displays the form to create a new movie (which renders the `movies/new-movie.hbs`), make sure you pass all the Celebrity from your database so your users can choose which ones are in the cast of the movie you're just creating (**hint**: You will have to use [select multiple](https://www.w3schools.com/tags/att_select_multiple.asp) tag)
...
Go back and review how you did this for the `Celebrity`. You'll need to:
...
3. In the route:
   - On the `Movie` model call the `findOne()` or `findById()` method to retrieve the details of a specific movie by its `id`
     - Don't forget you have `cast` as the array of celebrity `id`s, and we need to `populate()` in order to get the full data about the Celebrity 🎯
...
2. In that route:
   ...
   - Call the `Celebrity` model's `find()` to retrieve all Celebrity for the cast.
   - If everything is good, render the `movies/edit-movie` view
   - Pass the variable with the movie's details and all Celebrity into the view
...
And we are done! Now all `movies` CRUD features are implemented with a relationship between `movies` and `Celebrity`. As a **BONUS**, feel free to also add details view, edit and delete for the `Celebrity`.
